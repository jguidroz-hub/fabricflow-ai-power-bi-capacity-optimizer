...
interface CapacityOverviewItem {
  id: string;
  title?: string;
  status?: string;
  createdAt: string;
...
export default function CapacityOverviewPage() {
  const [items, setItems] = useState<CapacityOverviewItem[]>([]);
  ...
  const handleCreate = async () => {
    ...
    const res = await fetch('/api/capacity-overview', {
      method: 'POST',
  ...
  const handleDelete = async (id: string) => {
    await fetch(`/api/capacity-overview/${id}`, { method: 'DELETE' });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <p className="text-gray-600 mb-6">Real-time Power BI capacity monitoring</p>
